...
;; MONITOR INIT WILL CLEAR MEMORY FROM 0100H UP TO MONITOR START
;;  FOR USER WORK
;; -------------------------------------------------------------
...
_SMNAS: LD      C,A         ; CLEAR CONSECUTIVE-SPACE REF FLAG
        ...
        JP      C,_SMEFSE    ; FIELD WIDTH EXCEEDS DESTINATION BUFFER -- ERROR BAIL
...
SMPB0AD:.DW     SMPB0       ; POINTER - ADDRESS OF COMPONENT PARSING BUFFER 0
        ...
        ;; -------------------------------------------------------------
SMPBSZ: .EQU    20          ; COMPONENT BUFFER SIZE (SIZED TO FIT LARGEST FOR SIMPLICITY)
...
SMVAD:  .EQU    $
        ...
        LD      A,(HL)
        CP      0           ; IS BUFFER EMPTY (AN ENTER-ONLY USER INPUT)?
        JR      NZ,_SMVNZ   ; NO - PROCEED NORMALLY
        LD      HL,SMCURCM  ; GET DEFAULT 'CURRENT COMMAND' IF WE HAVE ONE
        LD      A,(HL)

        ;CALL    TOUPPER    ; NOTE - CALL HERE INSTEAD OF IN PARSE IF CMD-ONLY CASE CONVERSION
_SMVNZ: LD      HL,SMVCMDS  ; POINT HL TO ORDERED LIST OF VALID COMMANDS
        ...
        JP     _SMEINV     ; NO MATCH - DISPLAY ERROR MESSAGE
...
SMCMDW: CALL    PRINL
        ...
        ;; -------------------------------------------------------------
_SMESYN:LD      HL,SMERR00  ; LOAD 'SYNTAX ERROR' MESSAGE
        CALL    SMPRSE      ; DISPLAY AND EXIT
        RET

_SMEINV:LD      HL,SMERR01  ; LOAD 'INVALID COMMAND' MESSAGE
        CALL    SMPRSE      ; DISPLAY AND EXIT
        RET

_SMEFSE:LD      HL,SMERR02  ; 'PARAM WIDTH' ERROR
        CALL    SMPRSE      ;
        RET

_SMEMAL:LD      HL,SMERR04  ; LOAD 'MALFORMED RANGE' MESSAGE
        CALL    SMPRSE      ; DISPLAY AND EXIT
        RET

_SMEROS:LD      HL,SMERR05  ; LOAD 'RANGE OR SIZE' MESSAGE
        CALL    SMPRSE      ; DISPLAY AND EXIT
        RET
        ...
        ;;          CALL SMPRSE
        ;;
        ;; NOTE: THIS IS A BEAST ODDER THAN PLANNED BUT A) WORKS, AND
        ;;       B) HANDLES SOME CONSUMERS THAT ARRIVE HERE VIA CALL
        ;;       AND SOME THAT ARRIVE VIA JP.  THIS IS ONE OF THOSE
        ;;       UGLY DARK CORNERS BEST NOT TO POKE AROUND IN.  GO ON.
        ;; -------------------------------------------------------------
SMPRSE: .EQU    $
...
SMERR00:.TEXT   "SYNTAX ERROR",CR,LF,NULL
